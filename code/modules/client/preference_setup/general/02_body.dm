...
/datum/category_item/player_setup_item/physical/body/content(var/mob/user)
	...
	if(has_flag(mob_species, HAS_A_SKIN_TONE))
		. += "Цвет кожи: <a href='?src=\ref[src];skin_tone=1'>[-pref.s_tone + 35]/[mob_species.max_skin_tone()]</a><br>"

	. += "Нужны очки: <a href='?src=\ref[src];disabilities=[NEARSIGHTED]'><b>[pref.disabilities & NEARSIGHTED ? "Yes" : "No"]</b></a><br>"
	...
	. += "<br><a href='?src=\ref[src];toggle_preview_value=[EQUIP_PREVIEW_JOB]'>[pref.equip_preview_mob & EQUIP_PREVIEW_JOB ? "Скрыть одежду" : "Показать одежду"]</a>"
	. += "</td></tr></table>"
...
/datum/category_item/player_setup_item/physical/body/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["toggle_species_verbose"])
		...
		var/choice = input("На какие расы вы хотели бы взглянуть?") as null|anything in playable_species
		if(choice)
		...
		var/choice = input("Выберите расу вашего персонажа.") as null|anything in species_to_pick
		...
		var/new_hair = input(user, "Выберите цвет волос вашего персонажа:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_hair, pref.g_hair, pref.b_hair)) as color|null
		...
		var/new_h_style = input(user, "Выберите стиль волос вашего персонажа:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.h_style)  as null|anything in valid_hairstyles
		...
		if(!has_flag(mob_species, HAS_HAIR_COLOR))
			return TOPIC_NOACTION
		var/new_facial = input(user, "Выберите цвет волос на лице вашего персонажа:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_facial, pref.g_facial, pref.b_facial)) as color|null
		...
		if(!has_flag(mob_species, HAS_EYE_COLOR))
			return TOPIC_NOACTION
		var/new_eyes = input(user, "Выберите цвет глаз вашего персонажа:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_eyes, pref.g_eyes, pref.b_eyes)) as color|null
		...
		if(!has_flag(mob_species, HAS_BASE_SKIN_COLOURS))
			return TOPIC_NOACTION
		var/new_s_base = input(user, "Выберите цвет тела вашего персонажа:", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in mob_species.base_skin_colours
		...
		if(!has_flag(mob_species, HAS_A_SKIN_TONE))
			return TOPIC_NOACTION
		var/new_s_tone = input(user, "Выберите цвет кожи вашего персонажа. Чем ниже - тем ярче, чем выше - тем темнее. Диапозон от: 1 до [mob_species.max_skin_tone()]", CHARACTER_PREFERENCE_INPUT_TITLE, (-pref.s_tone) + 35) as num|null
		mob_species = all_species[pref.species]
		...
		var/new_f_style = input(user, "Выберите стиль волос на лице вашего персонажа:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.f_style)  as null|anything in valid_facialhairstyles
		...
		var/new_marking = input(user, "Выберите татуировки:", CHARACTER_PREFERENCE_INPUT_TITLE)  as null|anything in usable_markings
		...
		var/new_state = input(user, "В каком состоянии будет выбранная конечность?") as null|anything in choice_options
		...
		var/organ_name = input(user, "Какой внутренний орган вы хотите изменить?") as null|anything in list("Heart", "Eyes", "Lungs", "Liver", "Kidneys")
		...
		var/new_state = input(user, "В каком состоянии будет выбранный орган?") as null|anything in organ_choices
		if(!new_state) return
